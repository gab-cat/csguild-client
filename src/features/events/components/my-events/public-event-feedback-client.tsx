...
import { ArrowLeft, MessageSquare, Send, Star, Calendar, Clock, User, CheckCircle, Award, Shield } from 'lucide-react'
...
import { 
  ...
  useSubmitFeedbackResponsePublicMutation, 
  useSubmitOrganizerRatingPublicMutation 
} from '../../hooks'
...
interface PublicEventFeedbackClientProps {
  eventSlug: string
}

export function PublicEventFeedbackClient({ eventSlug }: PublicEventFeedbackClientProps) {
  ...
  const {
    ...
    isLoading,
    error
  } = useEventWithFeedbackFormPublic(eventSlug, token || '', userId || '')
  
  const submitFeedbackMutation = useSubmitFeedbackResponsePublicMutation()
  const submitOrganizerRatingMutation = useSubmitOrganizerRatingPublicMutation()
  ...
  useEffect(() => {
    if (!token || !userId) {
      showErrorToast('Invalid access link', 'Please use the link provided in your email.')
    }
  }, [token, userId])
  ...
  const handleSubmit = async (data: Record<string, string | number | string[]>) => {
    if (!feedbackForm?.id || !token || !userId) return
            ...
            <Card className="bg-gray-900/50 border-red-500/20">
              <CardContent className="p-12 text-center">
                <div className="max-w-md mx-auto space-y-4">
                  ...
                  >
                    This feedback link is invalid or has expired. Please use the link provided in your email.
                  </motion.p>
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
              ...
              <CardContent className="p-12 text-center">
                <div className="max-w-md mx-auto space-y-4">
                  ...
                  <motion.p 
                    ...
                    transition={{ duration: 0.4, delay: 0.4 }}
                  >
                    An error occurred while loading the event or your access token may have expired. Please try again later.
                  </motion.p>
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
